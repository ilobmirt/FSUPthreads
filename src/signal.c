     ...
     int sig;
{
  ...
  if (!sigismember((const sigset_t*)&new_signals, sig) && pthread_started)
#if (defined(_M_UNIX) && !defined(SCO5)) || defined(__linux__) || defined(__dos__)
    if (!is_in_kernel) {
#else
    if (!is_in_kernel && scp) {
      ...
      new_scp[sig] = &scp;
#elif defined (__dos__)
      new_scp[sig] =(struct context_t *) __djgpp_exception_state;
#else
      new_scp[sig] = scp;
#endif
      SET_KERNEL_FLAG;
      ...
      p->nscp = &scp;
#elif defined (__dos__)
      p->nscp =(struct context_t *)__djgpp_exception_state;
#else
      p->nscp = scp;
#endif
      ...
      sigaddset((sigset_t *)&new_signals, sig);
     ...
     int signaled, mode, activate;
{
   ...
   */
  sigdelset((sigset_t *)&new_signals, SIGALRM);
